...
export const UcpCheckoutResponseStatusSchema = z.enum([
    "error",
    "success",
]);
export type UcpCheckoutResponseStatus = z.infer<typeof UcpCheckoutResponseStatusSchema>;
...
export const JsonrpcSchema = z.enum([
    "2.0",
]);
...
export const A2AUcpMessageEnvelopeMethodSchema = z.enum([
    "message/send",
]);
export type A2AUcpMessageEnvelopeMethod = z.infer<typeof A2AUcpMessageEnvelopeMethodSchema>;
...
export const McpToolCallEnvelopeMethodSchema = z.enum([
    "tools/call",
]);
export type McpToolCallEnvelopeMethod = z.infer<typeof McpToolCallEnvelopeMethodSchema>;


export const BusinessLocationDestinationTypeSchema = z.enum([
    "business_location",
]);
export type BusinessLocationDestinationType = z.infer<typeof BusinessLocationDestinationTypeSchema>;


export const ShippingDestinationTypeSchema = z.enum([
    "shipping_address",
]);
export type ShippingDestinationType = z.infer<typeof ShippingDestinationTypeSchema>;
...
export const CardCredentialTypeSchema = z.enum([
    "card",
]);
...
export const NetworkTokenCredentialTypeSchema = z.enum([
    "network_token",
]);
export type NetworkTokenCredentialType = z.infer<typeof NetworkTokenCredentialTypeSchema>;
...
export const PanCredentialTypeSchema = z.enum([
    "pan",
]);
...
export const ConstraintsPropertySchema = z.object({
    ...
    "enum": z.array(z.any()).min(1).refine((items) => new Set(items.map((item) => JSON.stringify(item))).size === items.length, { message: "Array items must be unique (uniqueItems)" }).optional(),
});
...
export const CapabilityDiscoverySchema = z.object({
    ...
    "extends": z.union([z.array(z.string().regex(/^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/)).min(1), z.string().regex(/^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/)]).optional(),
    "name": z.string(),
    "schema": z.string().url(),
    "spec": z.string().url(),
...
export const A2ASchema = z.object({
    "endpoint": z.string().regex(/^https:\/\/[^\/?#\s\\@]+(?:\/[^?#\s\\]*[^\/?#\s\\])?$/).url(),
});
...
export const SignalsClassSchema = z.object({
    "dev.ucp.buyer_ip": z.string().optional(),
    "dev.ucp.user_agent": z.string().optional(),
}).catchall(z.any()).superRefine((value, ctx) => {for (const key of Object.keys(value)) {if (!/^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/.test(key)) {ctx.addIssue({ code: z.ZodIssueCode.custom, path: [key], message: `Property name ${JSON.stringify(key)} does not match the required pattern (propertyNames)` });}}});
...
export const LineItemResponseTotalSchema = z.object({
    "amount": z.number().int().gte(-9007199254740991).lte(9007199254740991),
    "display_text": z.string().optional(),
    "type": z.string(),
}).superRefine((value, ctx) => {for (const rule of [{"kind":"numeric","discriminator":"type","values":["discount","items_discount"],"negated":false,"required":[],"target":"amount","minimum":null,"maximum":null,"exclusiveMinimum":null,"exclusiveMaximum":0},{"kind":"numeric","discriminator":"type","values":["subtotal","fulfillment","tax","fee"],"negated":false,"required":[],"target":"amount","minimum":0,"maximum":null,"exclusiveMinimum":null,"exclusiveMaximum":null}]) {const record = value as Record<string, unknown>;const discriminatorVal = record[rule.discriminator];if (discriminatorVal === undefined) continue;const matches = (rule.values as readonly unknown[]).includes(discriminatorVal);if (rule.negated ? matches : !matches) continue;if (rule.kind === "required") {for (const field of rule.required) {if (!(field in record)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: [field], message: "Field is required by a conditional constraint" });}continue;}if (rule.target === null) continue;const target = record[rule.target];if (typeof target !== "number") continue;const invalid = (rule.minimum !== null && target < rule.minimum) || (rule.maximum !== null && target > rule.maximum) || (rule.exclusiveMinimum !== null && target <= rule.exclusiveMinimum) || (rule.exclusiveMaximum !== null && target >= rule.exclusiveMaximum);if (invalid) ctx.addIssue({ code: z.ZodIssueCode.custom, path: [rule.target], message: "Value violates a conditional numeric constraint" });}});
...
}).superRefine((value, ctx) => {for (const rule of [{"kind":"required","discriminator":"type","values":["error"],"negated":false,"required":["code","content","severity","type"],"target":null,"minimum":null,"maximum":null,"exclusiveMinimum":null,"exclusiveMaximum":null},{"kind":"required","discriminator":"type","values":["info"],"negated":false,"required":["content","type"],"target":null,"minimum":null,"maximum":null,"exclusiveMinimum":null,"exclusiveMaximum":null},{"kind":"required","discriminator":"type","values":["warning"],"negated":false,"required":["code","content","type"],"target":null,"minimum":null,"maximum":null,"exclusiveMinimum":null,"exclusiveMaximum":null}]) {const record = value as Record<string, unknown>;const discriminatorVal = record[rule.discriminator];if (discriminatorVal === undefined) continue;const matches = (rule.values as readonly unknown[]).includes(discriminatorVal);if (rule.negated ? matches : !matches) continue;if (rule.kind === "required") {for (const field of rule.required) {if (!(field in record)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: [field], message: "Field is required by a conditional constraint" });}continue;}if (rule.target === null) continue;const target = record[rule.target];if (typeof target !== "number") continue;const invalid = (rule.minimum !== null && target < rule.minimum) || (rule.maximum !== null && target > rule.maximum) || (rule.exclusiveMinimum !== null && target <= rule.exclusiveMinimum) || (rule.exclusiveMaximum !== null && target >= rule.exclusiveMaximum);if (invalid) ctx.addIssue({ code: z.ZodIssueCode.custom, path: [rule.target], message: "Value violates a conditional numeric constraint" });}});
...
}).catchall(z.any()).refine((value) => Object.keys(value).length >= 1, { message: "Object must contain at least 1 property(ies) (minProperties)" });
...
export type FulfillmentOptionElement = z.infer<typeof FulfillmentOptionElementSchema>;
...
}).superRefine((value, ctx) => {for (const rule of [{"kind":"required","discriminator":"has_next_page","values":[true],"negated":false,"required":["cursor"],"target":null,"minimum":null,"maximum":null,"exclusiveMinimum":null,"exclusiveMaximum":null}]) {const record = value as Record<string, unknown>;const discriminatorVal = record[rule.discriminator];if (discriminatorVal === undefined) continue;const matches = (rule.values as readonly unknown[]).includes(discriminatorVal);if (rule.negated ? matches : !matches) continue;if (rule.kind === "required") {for (const field of rule.required) {if (!(field in record)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: [field], message: "Field is required by a conditional constraint" });}continue;}if (rule.target === null) continue;const target = record[rule.target];if (typeof target !== "number") continue;const invalid = (rule.minimum !== null && target < rule.minimum) || (rule.maximum !== null && target > rule.maximum) || (rule.exclusiveMinimum !== null && target <= rule.exclusiveMinimum) || (rule.exclusiveMaximum !== null && target >= rule.exclusiveMaximum);if (invalid) ctx.addIssue({ code: z.ZodIssueCode.custom, path: [rule.target], message: "Value violates a conditional numeric constraint" });}});
...
export const CenterClassSchema = z.object({
    "latitude": z.number().gte(-90).lte(90),
    "longitude": z.number().gte(-180).lte(180),
});
export type CenterClass = z.infer<typeof CenterClassSchema>;
export const GeoSchema = CenterClassSchema;
export type Geo = CenterClass;


export const HoursSchema = z.object({
    "open_at": z.string().regex(/(?:[Zz]|[+-](?:[01][0-9]|2[0-3]):[0-5][0-9])$/).datetime({ offset: true }),
});
...
export const ExceptionHourElementSchema = z.object({
    ...
    "opens": z.string().regex(/^([01][0-9]|2[0-3]):[0-5][0-9]$/).optional(),
    "title": z.string().optional(),
...
export const LocationSchema = z.object({
    ...
    "id": z.string(),
    "name": z.string(),
    "amenities": z.record(z.string(), AmenitySchema).refine((value) => Object.keys(value).every((key) => /^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/.test(key)), { message: "Record keys must match the required pattern (propertyNames)" }).optional(),
    "exception_hours": z.array(ExceptionHourElementSchema).optional(),
    "geo": CenterClassSchema.optional(),
...
export type PaymentInstrumentSplitPayments = z.infer<typeof PaymentInstrumentSplitPaymentsSchema>;
...
export type EmbeddedTransportConfig = z.infer<typeof EmbeddedTransportConfigSchema>;
...
export type FulfillmentDestinationFilter = z.infer<typeof FulfillmentDestinationFilterSchema>;
...
export type BusinessLocationDestination = z.infer<typeof BusinessLocationDestinationSchema>;
...
export type NetworkTokenCredential = z.infer<typeof NetworkTokenCredentialSchema>;
...
export type RequestConstraintsProperty = z.infer<typeof RequestConstraintsPropertySchema>;
...
export const ConstraintsElementSchema = z.object({
    "anyOf": z.array(z.record(z.string(), z.any())).min(1).optional(),
    "properties": z.record(z.string(), ConstraintsPropertySchema).refine((value) => Object.keys(value).length >= 1, { message: "Object must contain at least 1 property(ies) (minProperties)" }).optional(),
    "required": z.array(z.string()).min(1).refine((items) => new Set(items.map((item) => JSON.stringify(item))).size === items.length, { message: "Array items must be unique (uniqueItems)" }).optional(),
}).passthrough();
...
export const ContextSchema = z.object({
    ...
    "eligibility": z.array(z.string()).refine((items) => new Set(items.map((item) => JSON.stringify(item))).size === items.length, { message: "Array items must be unique (uniqueItems)" }).optional(),
...
export const CheckoutResponseTotalSchema = z.object({
    "amount": z.number().int().gte(-9007199254740991).lte(9007199254740991),
    "display_text": z.string().optional(),
    "type": z.string(),
    "lines": z.array(TotalLineSchema).optional(),
}).superRefine((value, ctx) => {for (const rule of [{"kind":"required","discriminator":"type","values":["subtotal","items_discount","discount","fulfillment","tax","fee","total"],"negated":true,"required":["display_text"],"target":null,"minimum":null,"maximum":null,"exclusiveMinimum":null,"exclusiveMaximum":null}]) {const record = value as Record<string, unknown>;const discriminatorVal = record[rule.discriminator];if (discriminatorVal === undefined) continue;const matches = (rule.values as readonly unknown[]).includes(discriminatorVal);if (rule.negated ? matches : !matches) continue;if (rule.kind === "required") {for (const field of rule.required) {if (!(field in record)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: [field], message: "Field is required by a conditional constraint" });}continue;}if (rule.target === null) continue;const target = record[rule.target];if (typeof target !== "number") continue;const invalid = (rule.minimum !== null && target < rule.minimum) || (rule.maximum !== null && target > rule.maximum) || (rule.exclusiveMinimum !== null && target <= rule.exclusiveMinimum) || (rule.exclusiveMaximum !== null && target >= rule.exclusiveMaximum);if (invalid) ctx.addIssue({ code: z.ZodIssueCode.custom, path: [rule.target], message: "Value violates a conditional numeric constraint" });}});
...
export const ConsentValueSchema = z.object({
    "granted": z.boolean(),
    "segments": z.record(z.string(), SegmentValueSchema).refine((value) => Object.keys(value).every((key) => /^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/.test(key)), { message: "Record keys must match the required pattern (propertyNames)" }).optional(),
    "source": SourceSchema,
});
...
export const BuyerConsentSchema = z.object({
    ...
    "granted": z.boolean(),
    "links": z.array(LinkSchema).optional(),
    "segments": z.record(z.string(), ConsentSegmentSchema).refine((value) => Object.keys(value).every((key) => /^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/.test(key)), { message: "Record keys must match the required pattern (propertyNames)" }).optional(),
    "source": SourceSchema,
});
...
export const LocationFilterSchema = z.object({
    ...
    "items": z.array(z.string()).min(1).refine((items) => new Set(items.map((item) => JSON.stringify(item))).size === items.length, { message: "Array items must be unique (uniqueItems)" }).optional(),
});
export type LocationFilter = z.infer<typeof LocationFilterSchema>;

export const LocationServesSchema = z.object({
    "address": AddressSchema.optional(),
    "point": CenterClassSchema.optional(),
}).catchall(z.any()).refine((value) => Object.keys(value).length >= 1, { message: "Object must contain at least 1 property(ies) (minProperties)" });
...
export type OrderPaymentWithAcceptedTerm = z.infer<typeof OrderPaymentWithAcceptedTermSchema>;
...
export type CheckoutWithPaymentTermsPayment = z.infer<typeof CheckoutWithPaymentTermsPaymentSchema>;
...
export type CheckoutWithSplitPaymentsPayment = z.infer<typeof CheckoutWithSplitPaymentsPaymentSchema>;
...
export type EmbeddedProtocolMessageEnvelope = z.infer<typeof EmbeddedProtocolMessageEnvelopeSchema>;
...
export const BusinessSplitPaymentsConfigSchema = z.object({
    "allowed_combinations": z.array(z.array(AllowedCombinationElementSchema)).min(1),
});
export type BusinessSplitPaymentsConfig = z.infer<typeof BusinessSplitPaymentsConfigSchema>;
...
export const RequestConstraintsSchema = z.object({
    ...
    "path": z.string().optional(),
    "properties": z.record(z.string(), RequestConstraintsPropertySchema).refine((value) => Object.keys(value).length >= 1, { message: "Object must contain at least 1 property(ies) (minProperties)" }).optional(),
    "required": z.array(z.string()).min(1).refine((items) => new Set(items.map((item) => JSON.stringify(item))).size === items.length, { message: "Array items must be unique (uniqueItems)" }).optional(),
});
...
export type AvailablePaymentInstrument = z.infer<typeof AvailablePaymentInstrumentSchema>;
...
export type A2AUcpMessageEnvelopeParams = z.infer<typeof A2AUcpMessageEnvelopeParamsSchema>;
...
export const PaymentHandlerResponseSchema = z.object({
    "available_instruments": z.array(AvailablePaymentInstrumentSchema).min(1).optional(),
    "config": z.record(z.string(), z.any()).optional(),
    "id": z.string(),
    "schema": z.string().url().optional(),
    "spec": z.string().url().optional(),
...
export const UcpCheckoutResponseSchema = z.object({
    "capabilities": z.record(z.string(), z.array(CapabilityResponseSchema)).refine((value) => Object.keys(value).every((key) => /^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/.test(key)), { message: "Record keys must match the required pattern (propertyNames)" }).optional(),
    "map_order": z.string().optional(),
    "payment_handlers": z.record(z.string(), z.array(PaymentHandlerResponseSchema)).refine((value) => Object.keys(value).every((key) => /^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/.test(key)), { message: "Record keys must match the required pattern (propertyNames)" }),
    "services": z.record(z.string(), z.array(ServiceResponseSchema)).refine((value) => Object.keys(value).every((key) => /^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/.test(key)), { message: "Record keys must match the required pattern (propertyNames)" }).optional(),
...
export const UcpResponseSchema = z.object({
    "capabilities": z.record(z.string(), z.array(CapabilityResponseSchema)).refine((value) => Object.keys(value).every((key) => /^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/.test(key)), { message: "Record keys must match the required pattern (propertyNames)" }).optional(),
    "map_order": z.string().optional(),
    "payment_handlers": z.record(z.string(), z.array(PaymentHandlerResponseSchema)).refine((value) => Object.keys(value).every((key) => /^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/.test(key)), { message: "Record keys must match the required pattern (propertyNames)" }).optional(),
    "services": z.record(z.string(), z.array(ServiceResponseSchema)).refine((value) => Object.keys(value).every((key) => /^[a-z](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9_-]*[a-z0-9_])?)+$/.test(key)), { message: "Record keys must match the required pattern (propertyNames)" }).optional(),
    "status": UcpCheckoutResponseStatusSchema.optional(),
    "version": z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
});
...
export const CheckoutWithBuyerConsentResponseSchema = z.object({
    ...
    "totals": z.array(CheckoutResponseTotalSchema).superRefine((items, ctx) => {for (const rule of [{"property":"type","value":"subtotal","min":1,"max":1},{"property":"type","value":"total","min":1,"max":1}]) {const matches = items.filter((item) => item != null && (item as Record<string, unknown>)[rule.property] === rule.value).length;if (rule.min !== undefined && matches < rule.min) {ctx.addIssue({ code: z.ZodIssueCode.custom, message: `Array must contain at least ${rule.min} item(s) where ${rule.property} = ${JSON.stringify(rule.value)} (minContains)` });}if (rule.max !== undefined && matches > rule.max) {ctx.addIssue({ code: z.ZodIssueCode.custom, message: `Array must contain at most ${rule.max} item(s) where ${rule.property} = ${JSON.stringify(rule.value)} (maxContains)` });}}}),
...
export type CheckoutWithPaymentTerms = z.infer<typeof CheckoutWithPaymentTermsSchema>;
...
export type CheckoutWithSplitPayments = z.infer<typeof CheckoutWithSplitPaymentsSchema>;
...
export type McpToolCallEnvelopeParams = z.infer<typeof McpToolCallEnvelopeParamsSchema>;
